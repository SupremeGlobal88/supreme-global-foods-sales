...
export default function OrdersPage() {
  ...
  function canEditOrder(order: any): boolean {
    // Only pending orders can be edited. Admin can edit any pending order.
    // Sales rep can only edit their own pending orders.
          ...
          <div className="card-surface p-8 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto" style={{ borderRadius: 16 }}>
            ...
            {editingOrder && (
              <div className="mb-4 p-3 rounded-lg text-sm" style={{ backgroundColor: "rgba(212, 168, 67, 0.08)", border: "1px solid rgba(212, 168, 67, 0.2)", color: "#D4A843" }}>
                <Info className="w-4 h-4 inline mr-2" /> Editing a pending order will restore old stock and re-deduct new quantities.
              </div>
            )}
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={formData.customerId} onChange={(e) => handleCustomerSelect(parseInt(e.target.value))} className="input-field" required disabled={!!editingOrder}>
